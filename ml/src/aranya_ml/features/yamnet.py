...
def validate_yamnet_assumptions(
    sample_rate_hz: int, window_seconds: float, hop_seconds: float
) -> None:
    if (
        sample_rate_hz != YAMNET_SAMPLE_RATE
        or abs(window_seconds - YAMNET_WINDOW_SECONDS) > 1e-9
        or abs(hop_seconds - YAMNET_HOP_SECONDS) > 1e-9
    ):
        raise ValueError("settings do not mirror current YAMNet assumptions")
...
def load_local_saved_model(model_dir: str) -> Any:
    try:
        ...
        raise RuntimeError("TensorFlow is required; install only after approval") from exc
...
def resolve_embedding_output(outputs: Any) -> Any:
    ...
    for candidate in candidates:
        ...
        if shape is not None and len(shape) and shape[-1] == YAMNET_EMBEDDING_DIM:
            return candidate
...
def embedding_array(output: Any) -> Any:
    try:
        ...
        raise RuntimeError("NumPy is required to materialize embeddings") from exc
    array = output.numpy() if hasattr(output, "numpy") else np.asarray(output)
    if array.shape[-1] != YAMNET_EMBEDDING_DIM:
